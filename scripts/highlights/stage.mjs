...
import {
  abortBuildingPromotionTransaction,
  beginPromotionTransaction,
  descriptorPromotionKey,
...
export async function readStageManifest(
  manifestPath,
  { repoRoot = process.cwd(), allowedExtensionIds = [] } = {},
) {
  ...
  try {
    ...
    throw new Error(
      `cannot read stage manifest ${absoluteManifest}: ${error.message}`,
    );
  }
  validateManifestShape(manifest);
  const computedStageDigest = computeStageManifestDigest(manifest);
  if (manifest.stageDigest !== computedStageDigest)
    throw new Error("stage manifest digest does not match manifest content");
  if (
    path.basename(stageRealDir) !== manifest.stageDigest.slice("sha256:".length)
  ) {
    throw new Error("stage directory must be named by its stage digest");
  }
  if (manifest.qa.status !== "accepted")
    throw new Error("stage promotion requires accepted QA");

  const scenarioPath = await resolveStageRegularFile(
    stageDir,
    stageRealDir,
  ...
  if (
    computeScenarioDigest(scenario, { allowedExtensionIds }) !==
    manifest.scenario.digest
  ) {
    throw new Error("scenario source digest does not match staged scenario");
  }
  if (scenario.seed.recipe !== manifest.provenance.seedId) {
    throw new Error(
      "staged provenance seed identity does not match declarative scenario seed",
    );
  }
  const capturePath = await resolveStageRegularFile(
    stageDir,
    stageRealDir,
  ...
  if (`sha256:${await hashFile(capturePath)}` !== manifest.capture.digest)
    throw new Error("capture digest does not match staged raw master");
  const reportPath = await resolveStageRegularFile(
    stageDir,
    stageRealDir,
    manifest.qa.reportPath,
    "QA report",
  );
  const reportBytes = await fs.readFile(reportPath);
  if (
    `sha256:${createHash("sha256").update(reportBytes).digest("hex")}` !==
    manifest.qa.reportDigest
  )
    throw new Error("QA report digest does not match staged report");
  let qaReport;
  try {
  ...
  if (
    ...
    (qaReport.status !== "accepted" && qaReport.passed !== true)
  ) {
  ...
  for (const form of ["desktop", "mobile"]) {
    ...
    for (const kind of ASSET_KEYS) {
      ...
      const filePath = await resolveStageRegularFile(
        stageDir,
        stageRealDir,
        record.path,
        `${form} ${kind}`,
      ...
      if (seenPaths.has(filePath))
        throw new Error(`stage asset path is reused: ${record.path}`);
      ...
      if (
        ...
        throw new Error(
          `${form} ${kind} hash/bytes do not match stage manifest`,
        );
      }
...
export async function readReviewManifest(
  ...
  try {
    ...
    throw new Error(
      `cannot read review manifest ${absoluteManifest}: ${error.message}`,
    );
  }
  ...
  if (scenario.seed.recipe !== manifest.provenance.seedId) {
    throw new Error(
      "review provenance seed identity does not match declarative scenario seed",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "review QA report must record technical_pass with passed=true",
    );
  }
  ...
  for (const kind of ASSET_KEYS) {
    ...
    if (seenPaths.has(filePath))
      throw new Error(`review asset path is reused: ${record.path}`);
    seenPaths.add(filePath);
    const stat = await fs.lstat(filePath);
    if (
      stat.size !== record.bytes ||
      ...
      throw new Error(
        `${form} ${kind} hash/bytes do not match review manifest`,
      );
    }
    assets[form][kind] = { record, filePath };
  }
...
export async function promoteReviewedHighlight({
  ...
  if (typeof acceptedBy !== "string" || !REVIEWER_ID_PATTERN.test(acceptedBy)) {
    throw new Error(
      "review promotion requires acceptedBy as a stable reviewer id (--accept-reviewed-by)",
    );
  }
  ...
  if (!desktopManifestPath)
    throw new Error("desktop review manifest path is required");
  const desktop = await readReviewManifest(desktopManifestPath, {
    repoRoot,
    allowedExtensionIds,
  });
  if (desktop.form !== "desktop")
    throw new Error("desktop review manifest must use the desktop profile");
  const mobile = mobileManifestPath
    ? await readReviewManifest(mobileManifestPath, {
        repoRoot,
        allowedExtensionIds,
      })
    : null;
  if (mobile && mobile.form !== "mobile")
    throw new Error(
      "mobile review manifest must use the native-mobile profile",
    );
  if (desktop.mobileRequired && !mobile) {
    throw new Error(
      "desktop scenario declares mobileRequired; a native-mobile review is required",
    );
  }
  if (!desktop.mobileRequired && mobile) {
    throw new Error(
      "desktop scenario does not declare mobileRequired; refusing an unexpected mobile review",
    );
  }
  ...
  if (dryRun) {
    return {
      ...
      reviewDigests: [
        desktop.manifest.stageDigest,
        ...(mobile ? [mobile.manifest.stageDigest] : []),
      ],
      acceptance,
...
export async function promoteStagedHighlight({
  ...
  const staged = await readStageManifest(manifestPath, {
    repoRoot,
    allowedExtensionIds,
  });
  return promoteWithLock({
...
async function promoteWithLock({
  staged,
  repoRoot,
  highlightsDir,
  allowedExtensionIds,
  ...
  const lockPath = path.join(
    canonicalHighlightsDir,
    `.promote-${manifest.highlight.id}.lock`,
  );
  const lock = await acquirePromotionLock(
    lockPath,
    canonicalHighlightsDir,
    manifest.highlight.id,
  );
  let result;
  let primaryError;
  try {
    const recovered = await recoverPromotionTransaction({
      highlightsDir: canonicalHighlightsDir,
      highlightId: manifest.highlight.id,
      computeTreeDigest: computeSourceDigest,
      validateCatalog: ({ destination, transactionRoot }) =>
    ...
    if (recovered?.promotionKey === promotionKey(manifest)) {
      result = recovered;
    } else {
      result = await promoteVerifiedStage({
        staged,
        repoRoot,
        highlightsDir: canonicalHighlightsDir,
        allowedExtensionIds,
  ...
  try {
    ...
    cleanupError = new Error(
      `promotion lock cleanup failed for ${manifest.highlight.id}: ${error.message}`,
      { cause: error },
    );
  }
  if (primaryError && cleanupError) {
    throw new AggregateError(
      ...
      "Highlight promotion failed and promotion lock cleanup failed",
    );
  }
...
async function acquirePromotionLock(
  ...
  if (typeof startToken !== "string" || startToken === "") {
    throw new Error(
      `cannot prove promotion lock owner start token for PID ${process.pid}`,
    );
  }
  const owner = {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      const opened = await openPromotionLock(
        lockPath,
        canonicalHighlightsDir,
        highlightId,
      );
      ...
      try {
        ...
        throw new Error(
          `cannot prove promotion lock owner for ${lockPath}: ${ownerError.message}`,
          { cause: ownerError },
        );
      }
      if (currentToken === opened.lock.owner.startToken) {
        throw new Error(
          `active Highlight promotion lock for ${highlightId} is owned by PID ${opened.lock.owner.pid}: ${lockPath}`,
        );
      }
  ...
  throw new Error(
    `promotion lock remained contended after stale-lock retry: ${lockPath}`,
  );
}

async function openPromotionLock(
  lockPath,
  canonicalHighlightsDir,
  highlightId,
) {
  ...
  if (!pathStat.isFile() || pathStat.isSymbolicLink()) {
    throw new Error(
      `ambiguous promotion lock must be a regular non-symlink file: ${lockPath}`,
    );
  }
  const realPath = await fs.realpath(lockPath);
  if (path.dirname(realPath) !== canonicalHighlightsDir) {
    throw new Error(
      `ambiguous promotion lock escapes canonical Highlights directory: ${lockPath}`,
    );
  }
  let handle;
  try {
    ...
    if (error.code === "ENOENT") return null;
    if (error.code === "ELOOP")
      throw new Error(`ambiguous symlinked promotion lock: ${lockPath}`, {
        cause: error,
      });
    throw error;
  }
  try {
    ...
    if (stat.dev !== pathStat.dev || stat.ino !== pathStat.ino) {
      throw new Error(
        `ambiguous promotion lock changed while opening: ${lockPath}`,
      );
    }
    let lock;
    try {
      ...
      assertObjectKeys(
        lock,
        ["contract", "highlightId", "owner", "createdAt"],
        "promotion lock",
      );
      assertObjectKeys(
        lock.owner,
        ["pid", "startToken"],
        "promotion lock owner",
      );
      if (
      ...
      throw new Error(
        `malformed or ambiguous promotion lock ${lockPath}: ${error.message}`,
        { cause: error },
      );
    }
    return { lock, stat };
...
async function unlinkSamePromotionLock(lockPath, expectedStat, phase) {
  ...
  try {
    ...
    if (error.code === "ENOENT")
      throw new Error(
        `promotion lock disappeared before ${phase}: ${lockPath}`,
      );
    throw error;
  }
  if (
...
function buildAcceptedReviewStage({ desktop, mobile, acceptance }) {
  ...
  const source = {
    ...
    assets: {
      ...
      ...(mobile
        ? { mobile: structuredClone(mobile.manifest.assets.mobile) }
        : {}),
    },
    forms,
    acceptance: {
  ...
  const manifest = {
    ...source,
    stageDigest: computeStageManifestDigest(source),
  };
  return {
    manifest,
...
function assertReviewPair(desktop, mobile) {
  if (desktop.manifest.revision !== mobile.manifest.revision) {
    throw new Error(
      "paired mobile review revision does not match desktop review",
    );
  }
  ...
  if (canonicalJson(desktopHighlight) !== canonicalJson(mobileHighlight)) {
    throw new Error(
      "paired mobile review Highlight metadata does not match desktop review",
    );
  }
  for (const field of ["id", "title"]) {
    if (desktop.scenario[field] !== mobile.scenario[field]) {
      throw new Error(
        `paired mobile review scenario ${field} does not match desktop review`,
      );
    }
  }
  if (
    canonicalJson(desktop.scenario.seed) !== canonicalJson(mobile.scenario.seed)
  ) {
    throw new Error(
      "paired mobile review scenario seed does not match desktop review",
    );
  }
  ...
  for (const field of provenanceFields) {
    if (
      canonicalJson(desktop.manifest.provenance[field]) !==
      canonicalJson(mobile.manifest.provenance[field])
    ) {
      throw new Error(
        `paired mobile review provenance ${field} does not match desktop review`,
      );
    }
  }
  if (
    !sameRuntimePolicy(
      desktop.manifest.provenance.runtime,
      mobile.manifest.provenance.runtime,
    )
  ) {
    throw new Error(
...
async function promoteVerifiedStage({
  ...
  if (destinationStat && !destinationStat.isDirectory())
    throw new Error(`Highlight destination collision: ${destination}`);
  let existingDescriptor = null;
  if (destinationStat) {
    existingDescriptor = await parseHighlightDescriptor(
      path.join(destination, "highlight.json"),
    );
    if (existingDescriptor.status !== "queued")
      throw new Error(
        `cannot add revision to ${existingDescriptor.status} Highlight ${existingDescriptor.id}`,
      );
    if (
      (await exists(path.join(destination, "revisions", manifest.revision))) ||
      existingDescriptor.revision_history?.some(
        (entry) => entry.revision === manifest.revision,
      )
    ) {
      if (
        existingDescriptor.active_revision === manifest.revision &&
        descriptorPromotionKey(existingDescriptor) === promotionKey(manifest) &&
        (await computeSourceDigest(destination)) ===
          existingDescriptor.source_digest
      ) {
        const validationRoot = path.join(highlightsDir, "_transactions");
        await fs.mkdir(validationRoot, { recursive: true, mode: 0o700 });
        const validationRootStat = await fs.lstat(validationRoot);
        if (
          !validationRootStat.isDirectory() ||
          validationRootStat.isSymbolicLink() ||
          (await fs.realpath(validationRoot)) !== path.resolve(validationRoot)
        ) {
          throw new Error(
            "promotion validation root must be a canonical non-symlink directory",
          );
        }
        const validation = await validatePublishedPromotion({
          repoRoot,
          destination,
          transactionRoot: validationRoot,
          probe,
          now,
          allowedExtensionIds,
        });
        return {
          ...
          promotionKey: promotionKey(manifest),
          validation,
          recovered: true,
        };
      }
      throw new Error(
        `revision ${manifest.revision} already exists; refusing overwrite collision`,
      );
    }
  }
  ...
  const transaction = await beginPromotionTransaction({
    ...
    stageDigest: manifest.stageDigest,
    key: promotionKey(manifest),
    preimageSourceDigest: existingDescriptor?.source_digest ?? null,
    owner: lockOwner,
    now,
  });
  const candidateCatalog = path.join(transaction.root, "catalog");
  const candidateDir = path.join(candidateCatalog, manifest.highlight.id);
  try {
    await fs.mkdir(candidateCatalog, { recursive: true });
    if (destinationStat)
      await fs.cp(destination, candidateDir, {
        recursive: true,
        errorOnExist: true,
      });
    ...
    const desktop = await copyAssetSet(
      staged.assets.desktop,
      revisionDir,
      manifest.revision,
      "desktop",
    );
    const mobileAssets = staged.assets.mobile
      ? await copyAssetSet(
          staged.assets.mobile,
          revisionDir,
          manifest.revision,
          "mobile",
        )
      : null;
    const scenarioRecord = await copyTrackedFile(
      staged.scenarioPath,
      path.join(revisionDir, "scenario.json"),
      `revisions/${manifest.revision}/scenario.json`,
    );
    scenarioRecord.digest = manifest.scenario.digest;
    const mobileScenarioRecord = staged.mobileScenarioPath
      ? await copyTrackedFile(
          staged.mobileScenarioPath,
          path.join(revisionDir, "scenario.mobile.json"),
          `revisions/${manifest.revision}/scenario.mobile.json`,
        )
      : null;
    if (mobileScenarioRecord)
      mobileScenarioRecord.digest = manifest.forms.mobile.scenarioDigest;
    ...
    const provenanceRecord = fileRecord(
      `revisions/${manifest.revision}/provenance.json`,
      provenanceBytes,
    );
    ...
    const revisionHistory = existingDescriptor?.revision_history
      ...
      : existingDescriptor
        ? deriveLegacyRevisionHistory(existingDescriptor)
        : [];
    const activeFiles = [
      ...
      provenanceRecord,
    ].map(({ path: filePath, bytes, sha256 }) => ({
      path: filePath,
      bytes,
      sha256,
    }));
    ...
    await syncPromotionTree(candidateDir);
    await markPromotionPrepared(transaction, descriptor.source_digest);
    const recovered = await recoverPromotionTransaction({
      highlightsDir,
      highlightId: manifest.highlight.id,
      computeTreeDigest: computeSourceDigest,
    ...
    if (!recovered) {
      throw new Error("prepared promotion transaction did not publish");
    }
    return { ...recovered, validation };
  } catch (error) {
    try {
      await abortBuildingPromotionTransaction(transaction, highlightsDir);
    } catch (cleanupError) {
      throw new AggregateError(
        [error, cleanupError],
        "Highlight promotion failed and its building transaction could not be cleaned",
      );
    }
    throw error;
  }
...
async function validatePublishedPromotion({
  repoRoot,
  destination,
  transactionRoot,
  probe,
  ...
  const validationRoot = await fs.mkdtemp(
    path.join(transactionRoot, ".validation-"),
  );
  try {
    await fs.cp(
      destination,
      path.join(validationRoot, path.basename(destination)),
      { recursive: true, errorOnExist: true },
    );
    return await validateHighlights({
      repoRoot,
      highlightsDir: validationRoot,
      probe,
      now,
      allowedExtensionIds,
    });
...
function validateManifestShape(manifest) {
  ...
  if (manifest.schemaVersion !== STAGE_MANIFEST_VERSION)
    throw new Error(
      `stage manifest schemaVersion must be ${STAGE_MANIFEST_VERSION}`,
    );
  if (!DIGEST_PATTERN.test(manifest.stageDigest))
    throw new Error("stageDigest must be SHA-256");
  if (
    typeof manifest.revision !== "string" ||
    !/^[a-z0-9][a-z0-9._-]*$/.test(manifest.revision)
  )
    throw new Error("stage revision is invalid");
  ...
  if (!DIGEST_PATTERN.test(manifest.scenario.digest))
    throw new Error("scenario digest must be SHA-256");
  assertObjectKeys(manifest.capture, ["path", "digest"], "capture");
  assertSafeRelative(manifest.capture.path, "capture path");
  if (!DIGEST_PATTERN.test(manifest.capture.digest))
    throw new Error("capture digest must be SHA-256");
  assertObjectKeys(
    manifest.qa,
    ["status", "reportPath", "reportDigest", "acceptedAt"],
    "QA",
  );
  assertSafeRelative(manifest.qa.reportPath, "QA report path");
  if (
    !DIGEST_PATTERN.test(manifest.qa.reportDigest) ||
    !validDate(manifest.qa.acceptedAt)
  )
    throw new Error("QA requires report digest and acceptedAt");
  validateProvenance(manifest.provenance);
  if (!isObject(manifest.assets) || !manifest.assets.desktop)
    throw new Error("stage assets require desktop deliveries");
  ...
  for (const form of ["desktop", "mobile"]) {
    ...
    for (const kind of ASSET_KEYS)
      validateAssetRecord(manifest.assets[form][kind], form, kind);
  }
}
...
function validateReviewManifestShape(manifest) {
  ...
  if (manifest.contract !== REVIEW_STAGE_CONTRACT) {
    throw new Error(
      `review manifest contract must be ${REVIEW_STAGE_CONTRACT}`,
    );
  }
  if (manifest.schemaVersion !== REVIEW_STAGE_VERSION) {
    throw new Error(
      `review manifest schemaVersion must be ${REVIEW_STAGE_VERSION}`,
    );
  }
  if (!DIGEST_PATTERN.test(manifest.stageDigest))
    throw new Error("review stageDigest must be SHA-256");
  if (
    typeof manifest.revision !== "string" ||
    !/^[a-z0-9][a-z0-9._-]*$/.test(manifest.revision)
  ) {
  ...
  if (!DIGEST_PATTERN.test(manifest.scenario.digest))
    throw new Error("review scenario digest must be SHA-256");
  assertObjectKeys(manifest.capture, ["path", "digest"], "review capture");
  assertSafeRelative(manifest.capture.path, "review capture path");
  if (!DIGEST_PATTERN.test(manifest.capture.digest))
    throw new Error("review capture digest must be SHA-256");
  assertObjectKeys(
    manifest.qa,
    ["status", "passed", "reportPath", "reportDigest", "completedAt"],
    "review QA",
  );
  ...
  if (manifest.qa.status !== "technical_pass" || manifest.qa.passed !== true) {
    throw new Error(
      "review manifest requires technical_pass QA with passed=true",
    );
  }
  if (
    !DIGEST_PATTERN.test(manifest.qa.reportDigest) ||
    !validDate(manifest.qa.completedAt)
  ) {
  ...
  if (manifest.promotable !== false || manifest.readyForReview !== true) {
    throw new Error(
      "technical review must be non-promotable and ready for explicit review",
    );
  }
  const expectedReason =
    manifest.profile === "desktop"
      ? "explicit-acceptance-required"
      : "desktop-stage-required";
  if (manifest.reason !== expectedReason)
    throw new Error(`review reason must be ${expectedReason}`);
  const form = manifest.profile === "desktop" ? "desktop" : "mobile";
...
function validateReviewHighlightMetadata(highlight) {
  ...
  for (const field of [
    ...
    if (!Object.hasOwn(highlight, field))
      throw new Error(`review highlight is missing ${field}`);
  }
  ...
  if (
    highlight.mobileRequired !== undefined &&
    typeof highlight.mobileRequired !== "boolean"
  ) {
...
function assertReviewIdentity(manifest, scenario, delivery) {
  ...
  if (manifest.profile !== expectedProfile) {
    throw new Error(
      "review profile does not match declarative scenario profile",
    );
  }
  if (manifest.revision !== delivery.revision) {
    throw new Error(
      "review revision does not match declarative delivery revision",
    );
  }
  ...
  for (const field of [
    ...
    if (canonicalJson(actual) !== canonicalJson(expected[field])) {
      throw new Error(
        `review Highlight ${field} does not match declarative scenario delivery`,
      );
    }
  }
  for (const field of ["featureFlags", "docs"]) {
    if (
      canonicalJson(manifest.highlight[field]) !==
      canonicalJson(expected[field])
    ) {
      throw new Error(
        `review Highlight ${field} does not match declarative scenario delivery`,
      );
...
function validateHighlightMetadata(highlight) {
  ...
  if (
    ...
    !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(highlight.id)
  )
    throw new Error("stage Highlight id must be kebab-case");
  for (const field of ["title", "summary", "caption", "mobileDeclaration"])
    if (typeof highlight[field] !== "string" || !highlight[field].trim())
      throw new Error(`stage Highlight requires ${field}`);
  if (!/^\d+\.\d+\.\d+$/.test(highlight.releaseVersion))
    throw new Error("stage Highlight releaseVersion is invalid");
  if (
    !Array.isArray(highlight.featureFlags) ||
    highlight.featureFlags.length === 0 ||
    !highlight.featureFlags.every(
      (flag) =>
        typeof flag === "string" && /^[a-z0-9]+(?:[._-][a-z0-9]+)*$/.test(flag),
    )
  )
    throw new Error("stage Highlight requires featureFlags");
  assertObjectKeys(highlight.docs, ["page", "section"], "Highlight docs");
  if (
    typeof highlight.docs.page !== "string" ||
    typeof highlight.docs.section !== "string" ||
    !highlight.docs.section.trim()
  )
    throw new Error("stage Highlight requires docs ownership");
}
...
function validateProvenance(provenance, { runtimeRequired = false } = {}) {
  ...
  assertObjectKeys(provenance, [...common, ...pr, ...main], "provenance", {
    optional: true,
  });
  if (!["pr_head", "current_main"].includes(provenance.captureMode))
    throw new Error("stage provenance captureMode is invalid");
  if (
    !SHA_PATTERN.test(provenance.sourceSha) ||
    !validDate(provenance.capturedAt) ||
    typeof provenance.seedId !== "string" ||
    !provenance.seedId.trim() ||
    !DIGEST_PATTERN.test(provenance.seedDigest) ||
    typeof provenance.toolVersion !== "string" ||
    !provenance.toolVersion.trim()
  )
    throw new Error("stage provenance is incomplete");
  assertObjectKeys(
    provenance.landingAdapter,
    ["sourceSha", "contractVersion"],
    "landing adapter provenance",
  );
  if (!ADAPTER_SHA_PATTERN.test(provenance.landingAdapter.sourceSha)) {
    throw new Error(
      "landing adapter provenance requires exact lowercase Git SHA",
    );
  }
  if (
    typeof provenance.landingAdapter.contractVersion !== "string" ||
    !/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(
      provenance.landingAdapter.contractVersion,
    )
  ) {
    throw new Error(
      "landing adapter provenance requires a stable contract version",
    );
  }
  if (runtimeRequired) {
...
function validateQaRuntime(report, runtime, label) {
  if (canonicalJson(report.runtime) !== canonicalJson(runtime)) {
    throw new Error(
      `${label} runtime provenance does not match stage provenance`,
    );
  }
...
function validateAssetRecord(record, form, kind) {
  ...
  if (!record.path.endsWith(ASSET_EXTENSIONS[kind]))
    throw new Error(`${form} ${kind} must use ${ASSET_EXTENSIONS[kind]}`);
  if (
    !Number.isInteger(record.bytes) ||
    record.bytes <= 0 ||
    !/^[a-f0-9]{64}$/.test(record.sha256)
  )
    throw new Error(`${form} ${kind} requires exact bytes and SHA-256`);
  const expectedCodec =
    kind === "webm" ? "vp9" : kind === "mp4" ? "h264" : "webp";
  if (
    record.codec !== expectedCodec ||
    !Number.isInteger(record.width) ||
    !Number.isInteger(record.height) ||
    record.audio !== false
  )
    throw new Error(`${form} ${kind} media metadata is invalid`);
  const expectedDimensions = form === "desktop" ? [1920, 1200] : [1290, 2796];
  if (
    record.width !== expectedDimensions[0] ||
    record.height !== expectedDimensions[1]
  ) {
    throw new Error(
      `${form} ${kind} dimensions must be ${expectedDimensions[0]}x${expectedDimensions[1]}`,
    );
  }
  if (kind === "poster") {
    if (record.fps !== null || record.duration !== null)
      throw new Error(`${form} poster must not declare video timing`);
  } else if (
    record.fps !== 25 ||
...
function buildCompactProvenance(manifest) {
  ...
  return {
    ...
    ...(manifest.acceptance
      ? { acceptance: compactAcceptance(manifest.acceptance) }
      : {}),
  };
}
...
function buildDescriptor({
  ...
  return {
    ...
    ...(existingDescriptor?.published_at
      ? { published_at: existingDescriptor.published_at }
      : {}),
  };
}
...
function compactAcceptance(acceptance) {
  return {
    ...
    ...(acceptance.mobileReviewDigest
      ? { mobile_review_digest: acceptance.mobileReviewDigest }
      : {}),
  };
}
...
function compactReviewForms(forms) {
  return Object.fromEntries(
    Object.entries(forms).map(([form, record]) => [
      form,
      {
...
function deriveLegacyRevisionHistory(descriptor) {
  ...
  return files.length > 0
    ? [{ revision: descriptor.active_revision, files }]
    : [];
}
...
function resolveStageFile(stageDir, relativePath, label) {
  ...
  if (!relative || relative.startsWith("..") || path.isAbsolute(relative))
    throw new Error(`${label} must stay inside stage directory`);
  return filePath;
}

async function resolveStageRegularFile(
  stageDir,
  stageRealDir,
  relativePath,
  label,
  ...
  if (!stat?.isFile())
    throw new Error(`${label} must be a regular file inside stage directory`);
  const realPath = await fs.realpath(filePath);
  if (!isInside(stageRealDir, realPath) || realPath === stageRealDir) {
    throw new Error(
      `${label} must stay inside stage directory; symlinked parent directories are not allowed`,
    );
  }
  return realPath;
}
...
async function hashFile(filePath) {
  const stat = await fs.lstat(filePath).catch(() => null);
  if (!stat?.isFile())
    throw new Error(`stage source must be a regular file: ${filePath}`);
