...
async function readRegularBytes(
  ...
  try {
    ...
    throw new Error(
      `${label} cannot be opened without following symlinks: ${error.message}`,
    );
  }
  try {
...
function validateCaptureEvidenceIdentity(value, expectedPath) {
  ...
  if (
    ...
    throw new Error(
      "capture evidence identity contract or fixed path is invalid",
    );
  }
...
function validateCaptureContent(value, summary) {
  ...
  if (
    ...
    value.visibleDomText.some(
      (text) => typeof text !== "string" || !text.trim(),
    )
  ) {
    throw new Error(
      "visible DOM evidence must contain nonempty bounded text records",
    );
  }
  ...
  if (value.browserConsole.length === 0 && value.truncated.browserConsole) {
    throw new Error(
      "empty browserConsole needs an untruncated collection attestation",
    );
  }
  const actual = {
    visibleDomText: {
...
function validatePhaseRecord(value, captureReceipt) {
  ...
  if (
    ...
    throw new Error(
      "capture phase manifest contract, digest, or receipt link is invalid",
    );
  }
}
...
function validateHostSource(value) {
  ...
  if (
    ...
    canonicalJson(value.pre) !== canonicalJson(value.post)
  ) {
...
function compactHostSourceProof(proof) {
  return {
...
function validateBuildOutputIdentities(value, label) {
  requireExactKeys(value, BUILD_OUTPUT_KEYS, `${label} outputs`);
  const outputs = {};
  for (const key of BUILD_OUTPUT_KEYS) {
    ...
    const expectedKeys =
      key === "webDist"
        ...
        : ["digest", "bytes"];
    requireExactKeys(output, expectedKeys, `${label} ${key}`);
    requireDigest(output.digest, `${label} ${key} digest`);
    if (
      ...
      throw new Error(`${label} ${key} output identity is invalid`);
    }
    outputs[key] = structuredClone(output);
  }
  return outputs;
}

function validateManifestBuildOutputIdentities(value, label) {
  requireExactKeys(value, BUILD_OUTPUT_KEYS, `${label} outputs`);
  const compact = {};
  for (const key of BUILD_OUTPUT_KEYS) {
    const output = value[key];
    const expectedKeys =
      key === "webDist"
        ? ["path", "bytes", "fileCount", "digest", "files"]
        : ["path", "bytes", "digest"];
    requireExactKeys(output, expectedKeys, `${label} ${key}`);
    if (typeof output.path !== "string" || !path.isAbsolute(output.path)) {
      throw new Error(`${label} ${key} output path is invalid`);
    }
    if (key === "webDist" && !Array.isArray(output.files)) {
      throw new Error(`${label} webDist files must be an array`);
    }
    compact[key] = {
  ...
  return validateBuildOutputIdentities(compact, label);
}

function validateCompactBuildProof(value, label) {
  requireExactKeys(
    value,
  ...
  if (
    value.contract !== "kandev-highlight-build-provenance-v1" ||
    !/^[a-f0-9]{40}$/.test(value.sourceSha ?? "")
  ) {
    throw new Error(`${label} contract or source identity is invalid`);
  }
  requireDigest(value.manifestDigest, `${label} manifest digest`);
  return {
    contract: value.contract,
    manifestDigest: value.manifestDigest,
    sourceSha: value.sourceSha,
    outputs: validateBuildOutputIdentities(value.outputs, label),
  };
}

function buildOutputDigests(outputs) {
  return Object.fromEntries(
    BUILD_OUTPUT_KEYS.map((key) => [key, outputs[key].digest]),
  );
}

function validateBuildDigestProof(value, label) {
  requireExactKeys(value, ["manifestDigest", "sourceSha", "outputs"], label);
  requireDigest(value.manifestDigest, `${label} manifest digest`);
  if (!/^[a-f0-9]{40}$/.test(value.sourceSha ?? "")) {
    throw new Error(`${label} source identity is invalid`);
  }
  requireExactKeys(value.outputs, BUILD_OUTPUT_KEYS, `${label} outputs`);
  for (const key of BUILD_OUTPUT_KEYS)
    requireDigest(value.outputs[key], `${label} ${key}`);
  return structuredClone(value);
}

function validateApplicationBuildProof(value, label) {
  requireExactKeys(
    value,
    ["contract", "manifestDigest", "sourceSha", "outputs"],
    label,
  );
  if (value.contract !== "kandev-highlight-build-provenance-v1") {
    throw new Error(`${label} contract is invalid`);
  }
  return validateBuildDigestProof(
    {
      manifestDigest: value.manifestDigest,
      sourceSha: value.sourceSha,
...
function validateBuildManifest(value, sourceProof) {
  requireExactKeys(
    value,
    [
      "contract",
  ...
  if (
    ...
    canonicalJson(value.source) !== canonicalJson(sourceProof) ||
    value.manifestDigest !== digestBytes(canonicalJson(body))
  ) {
    throw new Error(
      "build provenance manifest contract, source, or digest is invalid",
    );
  }
  return {
    contract: value.contract,
    manifestDigest: value.manifestDigest,
    sourceSha: value.source.selectedSha,
    outputs: validateManifestBuildOutputIdentities(
      value.outputs,
      "build provenance manifest",
    ),
  };
...
function validateHostExecution(value) {
  ...
  const truthfulLogTruncation =
    value.log.truncated === value.log.discardedBytes > 0 &&
    (!value.log.truncated || value.log.capturedBytes === value.log.limitBytes);
  if (
    value.exitCode !== 0 ||
    value.signal !== null ||
    ...
    !Number.isSafeInteger(value.log.limitBytes) ||
    value.log.limitBytes <= 0 ||
    value.log.limitBytes > MAX_RUNTIME_LOG_BYTES ||
    !Number.isSafeInteger(value.log.capturedBytes) ||
    value.log.capturedBytes < 0 ||
    value.log.capturedBytes > value.log.limitBytes ||
    !Number.isSafeInteger(value.log.discardedBytes) ||
    value.log.discardedBytes < 0 ||
    typeof value.log.truncated !== "boolean" ||
    !truthfulLogTruncation
  ) {
    throw new Error(
      "runtime host execution did not exit cleanly within bounds",
    );
  }
  return value;
}
...
function validateHostTeardown(value) {
  ...
  if (
    Object.entries(value).some(
      ([key, nested]) => key !== "capture" && nested !== true,
    ) ||
...
function validateHostResult(value) {
  ...
  if (
    ...
    throw new Error(
      "runtime host result digest, success state, or teardown proof is invalid",
    );
  }
...
function validateCaptureReceiptAttestations(receipt) {
  ...
  if (
    receipt.buildVerification.contract !==
      "kandev-highlight-build-verification-v1" ||
    receipt.buildVerification.stable !== true ||
    canonicalJson(before) !== canonicalJson(after) ||
    before.manifestDigest !== captureBuild.manifestDigest ||
    before.sourceSha !== captureBuild.sourceSha ||
    canonicalJson(before.outputs) !==
      canonicalJson(buildOutputDigests(captureBuild.outputs))
  ) {
  ...
  for (const [label, sample] of Object.entries(receipt.storyMedia)) {
    requireExactKeys(
      sample,
      ["frameCount", "mediaTimeMs"],
      `capture story media ${label}`,
    );
    if (
  ...
  for (const [index, entry] of receipt.trustedInputLedger.entries()) {
    requireExactKeys(
      entry,
      trustedInputKeys,
      `capture trusted input ledger ${index}`,
    );
    requireExactKeys(
      entry.coordinates,
      ["x", "y"],
...
function validateRuntimeLog(bytes) {
  ...
  if (lines.length > MAX_RUNTIME_LOG_RECORDS) {
    throw new Error(
      `runtime log exceeds ${MAX_RUNTIME_LOG_RECORDS} record bound`,
    );
  }
  return lines.map((line, index) => {
    if (
      line.includes("\0") ||
      Buffer.byteLength(line) > MAX_RUNTIME_LOG_RECORD_BYTES
    ) {
      throw new Error(
        `runtime log record ${index} exceeds its typed text bound`,
      );
    }
...
export async function loadVerifiedRuntimeEvidence({
  ...
  if (path.resolve(attemptRoot ?? "") !== expectedAttempt) {
    throw new Error(
      "runtime evidence attemptRoot must use the fixed scenario/run path",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "runtime evidence attemptRoot must be a canonical non-symlink directory",
    );
  }
  if (
    ...
    throw new Error(
      "capture receipt does not bind the canonical scenario digest",
    );
  }
  ...
  if (canonicalJson(result.bundle) !== canonicalJson(expectedBundle)) {
    throw new Error(
      "runtime host bundle paths are not fixed to the selected run",
    );
  }
  ...
  if (
    ...
    canonicalJson(request.sourceProof) !==
      canonicalJson(captureReceipt.source) ||
    request.build.manifestDigest !== captureReceipt.build?.manifestDigest ||
    request.build.sourceSha !== captureReceipt.build?.sourceSha
  ) {
    throw new Error(
      "runtime worker request does not bind scenario, source, build, run, or fixed build path",
    );
  }
  const buildManifestRecord = await readJson(expectedBuildManifest, {
    root: path.join(externalRoot, "runtime-builds", runId),
    exactPath: expectedBuildManifest,
    label: "build provenance manifest",
  });
  const manifestBuild = validateBuildManifest(
    buildManifestRecord.value,
    request.sourceProof,
  );
  const requestBuild = validateCompactBuildProof(
    request.build,
    "runtime worker request build",
  );
  const captureBuild = validateCompactBuildProof(
  ...
  if (
    canonicalJson(manifestBuild) !== canonicalJson(requestBuild) ||
    canonicalJson(requestBuild) !== canonicalJson(captureBuild)
  ) {
    throw new Error(
      "build output identities do not match across manifest, runtime request, and capture receipt",
    );
  }
  const expectedSourceDigest = computeSourceCaptureDigest(
    request.source === "pr_head"
      ? {
          ...
          prHeadSha: request.sourceProof.selectedSha,
        }
      : {
          captureMode: request.source,
          sourceSha: request.sourceProof.selectedSha,
  ...
  if (captureReceipt.sourceDigest !== expectedSourceDigest) {
    throw new Error(
      "capture source identity digest does not match the independently recomputed source digest",
    );
  }
  if (
    canonicalJson(result.source.pre) !==
    canonicalJson(compactHostSourceProof(request.sourceProof))
  ) {
    throw new Error(
      "runtime host source evidence does not match worker request",
    );
  }
  ...
  const workerApplicationBuild = validateApplicationBuildProof(
    worker.applicationRuntime.build,
    "runtime worker application build",
  );
  if (
    worker.capture.scenarioDigest !== scenarioDigest ||
    worker.capture.sourceDigest !== captureReceipt.sourceDigest ||
    canonicalJson(worker.applicationRuntime) !==
      canonicalJson(captureReceipt.applicationRuntime) ||
    canonicalJson(workerApplicationBuild) !==
      canonicalJson({
        manifestDigest: requestBuild.manifestDigest,
        sourceSha: requestBuild.sourceSha,
        outputs: buildOutputDigests(requestBuild.outputs),
      })
  ) {
    throw new Error(
      "runtime worker result does not bind scenario, source, or application runtime",
    );
  }

  const expectedPhasePath = path.join(
    expectedAttempt,
    "evidence",
    "capture.json",
  );
  const expectedCapturePath = path.join(
    expectedAttempt,
    "capture",
    "evidence",
    "capture.json",
  ...
  if (
    ...
    canonicalJson(worker.capture.captureEvidence) !==
      canonicalJson(captureEvidenceIdentity) ||
    canonicalJson(result.capture?.captureEvidence) !==
      canonicalJson(captureEvidenceIdentity) ||
  ...
  if (
    ...
    throw new Error(
      "runtime host capture identity does not match selected attempt",
    );
  }
  const [phaseRecord, captureRecord, rawRecord, contentRecord] =
    await Promise.all([
  ...
  if (canonicalJson(captureRecord.value) !== canonicalJson(captureReceipt)) {
    throw new Error(
      "source capture manifest differs from recovered capture receipt",
    );
  }
  ...
  assertIdentity(
    rawRecord.identity,
    result.capture.rawMaster,
    "runtime host raw master",
  );
  const captureEvidence = validateCaptureContent(
    contentRecord.value,
  ...
  if (
    path.resolve(result.applicationRuntime.receiptPath ?? "") !==
    expectedReceiptPath
  ) {
    throw new Error(
      "application runtime receipt must use its fixed attempt path",
    );
  }
  ...
  const runtimeReceiptBuild = validateBuildDigestProof(
    receipt.build,
    "application runtime receipt build",
  );
  const expectedRuntimeReceiptBuild = {
    manifestDigest: requestBuild.manifestDigest,
    sourceSha: requestBuild.sourceSha,
    outputs: buildOutputDigests(requestBuild.outputs),
  };
  if (
    canonicalJson(runtimeReceiptBuild) !==
    canonicalJson(expectedRuntimeReceiptBuild)
  ) {
    throw new Error(
      "application runtime receipt build output identities do not match the runtime request",
    );
  }
  if (
    ...
    canonicalJson(receipt.workerResult) !==
      canonicalJson(result.workerResult) ||
    canonicalJson(receipt.log) !== canonicalJson(result.log) ||
    canonicalJson(receipt.preTeardown) !==
      canonicalJson(worker.applicationRuntime) ||
    ...
    canonicalJson(receipt.capture?.rawMaster) !==
      canonicalJson(rawRecord.identity) ||
  ...
  const logRecord = await readRegularBytes(expectedBundle.logPath, {
    ...
    label: "runtime log",
    maximumBytes: result.execution.log.limitBytes,
  });
