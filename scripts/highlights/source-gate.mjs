...
export async function verifySourceGate({
  ...
  const [headSha, currentMainSha, status] = await Promise.all([
    ...
    gitValue(runner, resolvedRoot, ["status", "--porcelain=v1", "--untracked-files=all"], "worktree status"),
  ]);
  if (!SHA_PATTERN.test(headSha)) throw new Error(`HEAD did not resolve to an exact Git SHA: ${headSha || "missing"}`);
  if (!SHA_PATTERN.test(currentMainSha)) {
    throw new Error(`origin/main did not resolve to an exact Git SHA: ${currentMainSha || "missing"}`);
  }
  if (status !== "") throw new Error(`source worktree must be clean; status: ${status}`);
  if (source === "current_main" && headSha !== currentMainSha) {
    throw new Error(`current_main requires HEAD ${headSha} to equal origin/main ${currentMainSha}`);
  }
  return {
...
export function assertExternalArtifactRoot({ artifactRoot, repoRoots = [] } = {}) {
  ...
  if (unsafeRoots.has(resolved) || resolved.split(path.sep).filter(Boolean).length < 2) {
  ...
  for (const repoRoot of repoRoots) {
    ...
    if (isWithin(repository, resolved)) {
      throw new Error(`artifact root must stay outside repository ${repository}: ${resolved}`);
    }
  }
  return resolved;
}
