...
describe("CanvasTaskCreateLauncher", () => {
  it("uses the same localized scratch preset from the settings entry point", () => {
    render(<CanvasTaskCreateLauncher workspaceId="workspace-1" presentation="settings" />);
  ...
  it("uses the identical preset from the sidebar entry point and follows the task route", () => {
    render(
      <TooltipProvider>
        <CanvasTaskCreateLauncher workspaceId="workspace-1" presentation="sidebar" />
      </TooltipProvider>,
    );

    fireEvent.click(screen.getByRole("button", { name: CREATE_CANVAS_LABEL }));
    const settingsPreset = mocks.dialogProps?.initialValues;
    fireEvent.click(screen.getByTestId("canvas-dialog-submit"));

    expect(settingsPreset).toEqual({
      title: CREATE_CANVAS_TITLE,
      description: CREATE_CANVAS_PROMPT,
      noRepository: true,
      preferLocalExecutor: true,
