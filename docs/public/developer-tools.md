...
Auto-start and auto-install are off for every language by default. These global settings are defaults; each task and language has its own **Inherit default**, **Keep warm**, or **Disabled** policy. The same settings page lets you show or hide each language in task status. Visibility is presentation-only: hiding a language does not start, stop, enable, disable, or otherwise change its task server. Bounded project discovery can activate an inherited default without requiring you to open a matching file, but discovery only inspects names and extensions. It does not install or start a server or evaluate project files.

Use **Start** to set **Keep warm** for that task and language. The server then remains warm across files, panels, sessions, browser reloads, and ordinary disconnects until **Stop**, task teardown, or another task policy transition. **Stop** sets **Disabled** so an editor or reconnect cannot reacquire the server. **Restart** preserves the policy, discards the current process and analysis, and creates one new generation; a project import may run again.

With **Show status bar** enabled under **Settings > Preferences > Appearance > Status Bar**, its compact task item remains visible while a relevant visible language is detected, configured, running, importing, queued, or failed, even when an unsupported file or another panel is active. Open it to see one compact collapsible per visible language. The collapsed summary keeps the language, state, and detection evidence visible; expand it for policy, lifecycle phase, reported work, elapsed time, generation, last action evidence, errors, and controls. Each language expands independently.

The task top bar provides the same always-discoverable control when the status bar is disabled, no supported file is active, or every detected language is hidden. An all-hidden disclosure links back to **Settings > General > Editors**. A supported Monaco editor keeps its current-language shortcut even when that language is hidden from aggregate task status.

Phone and touch-oriented tablet layouts expose the same visibility preference, inline collapsibles, task-level state, and controls in the existing **Status** drawer with touch-sized actions. The phone file viewer itself does not attach to or silently start a language server.
...
The task host owns protocol initialization, progress, and the process tree, so closing the last editor or browser attachment does not stop the server. A backend restart reattaches to a live task-host generation when possible; a task-environment restart necessarily creates a new generation. Task stop, archive, and delete stop that task's language servers. When another live task shares the departing owner's physical environment, Kandev transfers environment ownership and keeps the shared task host plus the other task's servers alive. Otherwise cleanup reaps the full server process tree; environment teardown always does. Stop and archive preserve the task policy so resume can reconcile it, while delete removes the task's language-server records.
