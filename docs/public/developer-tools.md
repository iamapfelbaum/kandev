...
Repository-root membership can update in place when the server supports workspace-folder changes. Moving the physical task workspace itself requires an explicit **Restart** because the process working directory and LSP root are initialize-time state; the current generation keeps its old scope until then. Browser document traffic is constrained to the canonical task workspace and authorized repository roots, including protection against traversal and symlink escapes.
