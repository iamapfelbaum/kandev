...
The task host owns protocol initialization, progress, and the process tree, so closing the last editor or browser attachment does not stop the server. A backend restart reattaches to a live task-host generation when possible; a task-environment restart necessarily creates a new generation. Task stop, archive, delete, and environment teardown reap the full server process tree. Stop and archive preserve the task policy so resume can reconcile it, while delete removes the task's language-server records.
