...
The backend will use durable session prompt history before it applies the task-description fallback. Then the frontend will correct two plan-tool names.
...
Add a bounded repository query for `task_session_prompt_seq.last_seq`. The query reports whether a session has accepted at least one user prompt.

Expose this query through the repository contract that the orchestrator uses. Keep message deletion behavior unchanged because the counter never decreases.

### Shared workflow prompt decision

Add an orchestrator helper that composes a workflow-entry prompt for one session. Use the task description only when the step prompt is empty and the prompt counter is zero.
...
If the composed ACP prompt is empty, let `autoStartStepPrompt` inspect any queued handoff first. Return without message creation or dispatch only when the merged prompt and attachments are empty.
...
- `AC-TASKS-WORKFLOW-STEP-AGENT-START-OWNERSHIP-003.4`: assert that an empty result creates no user row and no runtime prompt.
...
Move the idle task into an empty automatic-start step. Open the task and assert that the description appears in exactly one user message.
...
- Backend focused race suite: 9 tests passed.
...
- A broad guard can suppress non-empty step prompts. The guard applies only to the empty-step fallback.
