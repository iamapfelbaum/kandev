...
Open the standard task creation flow from both canvas entry points. Apply one
canvas preset without hiding the normal workflow and agent choices.

## In scope

- Add scratch source mode and local executor preference to task dialog presets.
- Add one localized `CanvasTaskCreateLauncher`.
- Add the sidebar `+` action and workspace settings primary action.
...
- Both entry points pass the same title, prompt, source mode, and executor
  preference.
