...
[org roles and scopes](../../auth/requirements/roles-and-scopes.md).

## Requirements

### REQ-WORKSPACES-MEMBERSHIP-001: Workspace visibility and membership

**Intent:** A team running one Kandev server needs a shared board: everyone
sees what everyone else is working on, opens a colleague's task, and takes it
over. The strict per-owner segregation that came before leaves no supported
path for that, and the only workaround is a shared login, which forfeits the
whole per-user layer.

#### Acceptance criteria

- **AC-WORKSPACES-MEMBERSHIP-001.1:** A workspace has a visibility of `org` or
  `private`. `org` means every member of the org reaches it with the scopes
  their org role grants; `private` means only the owner and explicitly added
  members reach it.
- **AC-WORKSPACES-MEMBERSHIP-001.2:** The org carries a default visibility for
  new workspaces, settable by a caller holding `org.settings.manage`.
- **AC-WORKSPACES-MEMBERSHIP-001.3:** Existing workspaces migrate to `private`.
  An upgrade never widens access to data that was private the moment before.
- **AC-WORKSPACES-MEMBERSHIP-001.4:** A `workspace_members` row names a user and
  a workspace role of `owner`, `collaborator` or `viewer`, and always outranks
  the org default in both directions.
- **AC-WORKSPACES-MEMBERSHIP-001.5:** Reach and permission stay separate
  questions. This system decides which workspaces a user reaches; scopes decide
  what they may do there.
- **AC-WORKSPACES-MEMBERSHIP-001.6:** A task carries a human assignee distinct
  from its agent assignee. Assignment is advisory, gates nothing, and takeover
  is reassign plus continue: no lock is taken and no session is torn down.
- **AC-WORKSPACES-MEMBERSHIP-001.7:** Every human action records the acting
  user, and no path falls back to the workspace owner when the actor is unknown.
- **AC-WORKSPACES-MEMBERSHIP-001.8:** Concurrent prompting is serialized through
  the existing message queue rather than blocked by a per-session human lock.
- **AC-WORKSPACES-MEMBERSHIP-001.9:** With multi-tenancy enabled, `org`
  visibility means the workspace's own org and nothing wider, and adding a
  member from another org is refused with 404.
...
- **What a member can do is decided by scopes, not by membership.** Reach and
  ...
  [roles and scopes](../../auth/requirements/roles-and-scopes.md) owns the second. This spec
...
- Workspace DTOs gain `visibility`, `member_count`, `viewer_role`, and
  `scopes` (see [roles and scopes](../../auth/requirements/roles-and-scopes.md)).
...
[roles and scopes](../../auth/requirements/roles-and-scopes.md). The two compose as:
